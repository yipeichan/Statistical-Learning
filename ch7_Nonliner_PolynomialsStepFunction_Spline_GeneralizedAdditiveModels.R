...
{plot(fitted(fita),col="red")
  points(fitted(fit), col="blue")}
...
se.bands=preds$fit + cbind(fit=0,lower=-2*preds$se,upper=2*preds$se)  #cbind Combine R Objects by Rows or Columns
se.bands[1:5,]
...
{matplot(age.grid,prob.bands,col="blue",lwd=c(2,1,1),lty=c(1,2,2),type="l",ylim=c(0,.1))
  points(jitter(age),I(wage>250)/10,pch="|",cex=.5,col="red")}
